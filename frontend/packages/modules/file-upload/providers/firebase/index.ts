import { storage } from '@app-launch-kit/modules/init/firebase/firebaseClient';
import {
  deleteObject,
  ref,
  uploadBytes,
...
export const Service: IFileUploadProvider = {
  async uploadMedia(
    ...
    const storageRef = storage().ref(`${mediaStoragePath}/${filename}`);
  ...
  async deleteMedia(
    ...
    try {
      const storageRef = storage().ref(`${mediaStoragePath}/${mediaUri}`);
